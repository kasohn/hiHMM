function [S, stats, mapS] = hiHmmp_model2( Y, hypers, numb, nums, numi, S0, odir, markers )
...
%            trellis and the marginal likelihood.
	...
	head = sprintf( 'train-hihmm-model2' );
	...
	if isfield(hypers, 'Mus')
		...
		sample.Mus = hiHmmSampleMultNormalMeans2( sample.S, Y, sample.K, hypers.sigma2, hypers.mu_0, hypers.sigma2_0 );
	end
	...
	while iter <= (numb + (nums-1)*numi)
		  ...
		  for c1 = rid
			  ...
			  Gc = sample.G{c1}(jid(1:nseg));
			   ...
			   parfor j = 1:nseg   % jid
				[ Snew, Gnew ] = Sample_hiddenState_model2( Y{c1}{jid(j)}, sample.S{c1}{jid(j)}, sample.G{c1}{jid(j)}, uc{j},  sample, hypers, c1, iter );
				Sc{j} = Snew;
				Gc{j} = Gnew;
			   end  %%% end of j 
		   ...
		   stats.jll(iter) = hiHmmMultNormalJointLogLikelihood2(sample.S, Y, sample.Beta, ...
...
function [ sample, uc ] = BreakSticks( Sc, sample, hypers, c1 )
	...
	%    while max( max(sample.Pi(:, end, :)) ) > min( cell2mat(u))     % Break the Pi{k} stick some more.
		        ...
		        for cc=1:C 
		                sample.Pi(bl,:,cc) = dirichlet_sample(sample.alpha0 * sample.Beta);
...
function [ Snew, Gnew ] = Sample_hiddenState_model2( Yj, Sj, Gj, uc, sample, hypers, c1, iter ) 
	%      Sj = sample.S{c1}{j};
	%      Yj = Y{c1}{j};
	      C = size( sample.Pi, 3 );
	      M = size( Yj, 1 );
	...
	%    stats.trellis(iter) = stats.trellis(iter) + sum(sum(dyn_prog(:,1)));
	    for k=1:sample.K
	        dyn_prog(k,1) = exp(-0.5*(Yj(:,1) - sample.Mus(:,k))'*(Yj(:,1) - sample.Mus(:,k))/hypers.sigma2) * dyn_prog(k,1);
	    end
	...
	%        stats.trellis(iter) = stats.trellis(iter) + sum(sum(A));
	        for k=1:sample.K
	            dyn_prog(k,t) = exp(-0.5*(Yj(:,t) - sample.Mus(:,k))'*(Yj(:,t) - sample.Mus(:,k))/hypers.sigma2) * dyn_prog(k,t);
	        end
