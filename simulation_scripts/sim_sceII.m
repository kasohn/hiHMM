...
for rr=1:nset
  ...
  for sf = [ 0.1, 0.3, 0.5 ]	%% noise variance across species 
    ...
    param.Pi = pi;
    Strue = S;
    fprintf(1, 'simulation_II_mat/%s%d_sf_%.1f_sigma%.1f_%d.mat\n', sce0, ss, sf, sigma0, rr );
    save( sprintf('simulation_II_mat/%s%d_sf_%.1f_sigma%.1f_%d.mat', sce0, ss, sf, sigma0, rr),'Y', 'Strue', 'param' );
